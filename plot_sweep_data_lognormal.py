...
print("\nEOC is: ")
print(eocBoch)
np.save("./results/" + exp_date + "/" + "rate_table_L1_data_sweep_lognormal.npy", eocBoch)
...
def get_slopes(array_2d, my_str="noisy", nplot=nplot, nplota=nplota, exp_date=exp_date, SHIFT=SHIFT):
    ...
    print(my_slopes)
    np.save("./results/" + exp_date + "/" + 'rate_ls_data_sweep_lognormal_' + my_str + '.npy', linefit)
...
def make_data_sweep_plot(my_errors, fig_num=0, my_str="noisy"):
    ...
    if FLAG_save_plots:
        if FLAG_WIDE:
            plt.savefig("./results/" + exp_date + "/" + 'data_sweep_wide_lognormal_' + my_str + '.pdf', format='pdf')
        else:
            plt.savefig("./results/" + exp_date + "/" + 'data_sweep_lognormal_' + my_str + '.pdf', format='pdf')
    plt.show()
...
def make_data_sweep_plot_fixed_noise(my_errors, noise_idx, noise_val):
    ...
    if FLAG_save_plots:
        if FLAG_WIDE:
            plt.savefig("./results/" + exp_date + "/" + 'data_sweep_wide_noise_lognormal' + str(noise_val) + '.pdf', format='pdf')
        else:
            plt.savefig("./results/" + exp_date + "/" + 'data_sweep_noise_lognormal' + str(noise_val) + '.pdf', format='pdf')
    plt.show()
...
def make_noise_fit_power(my_errors, x, d, model, fig_num=0, my_str="noisy"):
    ...
    if FLAG_save_plots:
        if FLAG_WIDE:
            plt.savefig("./results/" + exp_date + "/" + 'data_power_wide_lognormal_' + my_str + '.pdf', format='pdf')
        else:
            plt.savefig("./results/" + exp_date + "/" + 'data_power_lognormal_' + my_str + '.pdf', format='pdf')
    plt.show()
