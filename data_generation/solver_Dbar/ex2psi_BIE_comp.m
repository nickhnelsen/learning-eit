...
for kkk = 1:length(Kvec)
    ...
    % Compute Fourier representation of Calderon's exponential function exp(ikx)
    cald  = exp(1i*k*exp(1i*theta));
    Fcald = 1/(2*pi)*Dtheta*exp(-1i*Nvec.'*theta.')*cald;
    
    % Construct the operator to be inverted
%     eval(['load data/ex2Hk_', num2str(kkk), ' Hk']);
    load(['/media/nnelsen/SharedHDD2TB/datasets/eit/dbar/hmatrices/ex2Hk_', num2str(kkk), '.mat'], 'Hk');

    Oper = eye(length(Nvec)) + (S0 + Hk)*(DN - DN1);
    
    % Compute Fourier coefficients of the solution psi
    Fpsi_BIE(:,kkk) = Oper\Fcald;
    
    % Monitor the run
    if mod(kkk,100)==0
        disp(['Done (CGO traces) ', num2str(kkk), ' out of ', num2str(length(Kvec))])
%         save data/ex2psi_BIE Fpsi_BIE Nvec
        save('/media/nnelsen/SharedHDD2TB/datasets/eit/dbar/data/ex2psi_BIE.mat', 'Fpsi_BIE', 'Nvec');
    end
end
