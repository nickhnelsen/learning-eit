...
%       get_cond_data_three_phase.m, conductivityGRF.m, FEMconductivityGRF.m, mesh.mat
%
% Script to generate large Fourier basis truth set of ND maps
% Three phase conductivities
% Nicholas H. Nelsen Oct. 2025

function []=data_generation_fND_three_phase_script(seed,N_cond,N_solves,...
    tau_m,tau_p,al_m,al_p,rho_m,rho_p,val1_m,val1_p,val2_m,val2_p)
...
val1_m = str2double(val1_m);
val1_p = str2double(val1_p);
val2_m = str2double(val2_m);
val2_p = str2double(val2_p);
...
tau_cond = [tau_m;tau_p];       % min 12 max 18
alpha_cond = [al_m;al_p];       % min 4 max 5
rho = [rho_m;rho_p];            % Outer truncation radius to conductivity value 1; min .65 max .9
val1 = [val1_m;val1_p];         % phase 1 value; min 2 max 10
val2 = [val2_m;val2_p];         % phase 2 value; min 0.1 max 0.5
...
save_path = append("./eit_bin_fND_three_phase_",str0,str1,str2,str3);
...
val1_c = val1_m+(val1_p-val1_m).*rand(N_cond,1);
val2_c = val2_m+(val2_p-val2_m).*rand(N_cond,1);
for oo = 1:N_cond
    s2=tic;
    % Get raw data and gridded interpolant
    [raw_cond, interp_cond] = get_cond_data_three_phase(K_cond,tau_c(oo),al_c(oo),rho_c(oo),val1_c(oo),val2_c(oo));
    ...
    'val1','val2','seed','-v7.3','-nocompression')
    end
...
save(save_path,'kvec_array','ntd_array','cond_array','tau_cond','alpha_cond','rho',...
    'val1','val2','seed','-v7.3','-nocompression')
disp(['Done (full three phase data generation) ', 'in ', num2str(toc(s1)/3600), ' hours'])
rmpath('./solver/');
