...
# Robustness Plot: Err vs noise, varying sample size on linear linear scale
...
for d in [param_power,param_power_clean,param_log,param_log_clean]:
    for i, fit in enumerate(d, start=1):
        print(f"Curve {i}: E0 = {fit['E0']:.4f}, c = {fit['c']:.4f}, rho = {fit['rho']:.3f}")
        print("Done")


def make_noise_fit_plot(my_errors, x, d, model, fig_num=0, my_str="noisy"):
    """
    my_errors: (Noise, N_train, MeanOrStdev) array
    """
    plt.figure(fig_num)
    for i in range(len(N_list)):
        plt.loglog(x, model(x, **d[i]), ls='-', color='purple') # darkgray
...
# make_noise_fit_plot(y[...,0,:], s, param_power, model_power, 10, "noisy")
# make_noise_fit_plot(y[...,1,:], s, param_power_clean, model_power, 11, "clean")
# make_noise_fit_plot(y[...,0,:], s, param_log, model_log, 20, "noisy")
# make_noise_fit_plot(y[...,1,:], s, param_log_clean, model_log, 21, "clean")
...
def make_noise_fit_log(my_errors, x, d, model, fig_num=0, my_str="noisy"):
    ...
    plt.figure(fig_num)
    for i in range(len(N_list)):
        plt.loglog(xplot, model(x, **d[i]) - d[i]['E0'], ls='-', color='purple') # darkgray
    ...
    if FLAG_save_plots:
        if FLAG_WIDE:
            plt.savefig("./results/" + exp_date + "/" + 'noise_log_wide_' + my_str + '.pdf', format='pdf')
        else:
            plt.savefig("./results/" + exp_date + "/" + 'noise_log_' + my_str + '.pdf', format='pdf')
    plt.show()

make_noise_fit_log(y[...,0,:], s, param_log, model_log, 8, "clean")
make_noise_fit_log(y[...,1,:], s, param_log_clean, model_log, 9, "clean")
